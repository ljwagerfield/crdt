...
Two-way merge detection of any data type can be achieved by attaching any CvRDT as a header to the underling payload, whether it is a CvRDT or not. Any CvRDT that supports unique increments can be used since **all CvRDTs will produce the same graph given the same set of concurrent unique increments.**
